...
const INFOS = [
  { code:'email',    icon:FiMail,     label:'Email',        value:'hamidrobgo009@gmail.com',    href:'mailto:hamidrobgo009@gmail.com' },
  { code:'location', icon:FiMapPin,   label:'Localisation', value:'Cocody Saint-Jean, Abidjan', href:'#' },
  { code:'linkedin', icon:FiLinkedin, label:'LinkedIn',     value:'linkedin.com/in/hamid',       href:'https://linkedin.com' },
  { code:'github',   icon:FiGithub,   label:'GitHub',       value:'github.com/dimah',            href:'https://github.com' },
];

const LANGS_BASE = [['Français','Langue maternelle'],['Anglais','B2 · TOEIC 885']];
...
export default function ContactPage() {
  const { tr } = useLang(); const c = tr.contact;
  const { isAdmin } = useAdmin();
  const [form, setForm]     = useState(INIT);
  const [status, setStatus] = useState('idle');

  /* Traduit les labels des infos de contact */
  const infoLabels = useTranslatedArray(INFOS.map(i => i.label));
  /* Traduit les langues */
  const langLabels = useTranslatedArray(LANGS_BASE.map(l => l[0]));
  const langLevels = useTranslatedArray(LANGS_BASE.map(l => l[1]));
  ...
  const inputStyle = { width:'100%', background:'transparent', border:'none', borderBottom:'1px solid rgb(var(--ink)/0.15)', padding:'10px 0', fontSize:'14px', fontFamily:"'Jost',sans-serif", fontWeight:300, color:'rgb(var(--ink))', outline:'none', transition:'border-color .2s' };
    ...
    <main>
      <section style={{ padding:'80px 40px', borderBottom:'1px solid rgb(var(--ink)/0.08)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'60px' }} className="max-xl:block">
          <motion.h1 {...up(0.15)} style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'clamp(52px,6vw,96px)', fontWeight:700, lineHeight:0.92, letterSpacing:'-0.04em', whiteSpace:'pre-line' }}>{c.title}</motion.h1>
      ...
      <section style={{ padding:'60px 40px', display:'grid', gridTemplateColumns:'320px 1fr', gap:'80px' }} className="max-xl:block">
        <motion.aside {...up(0.2)} style={{ display:'flex', flexDirection:'column', gap:'28px' }}>
          {INFOS.map(({ code, icon:Icon, value, href }, i) => (
            <div key={code} style={{ display:'flex', gap:'16px', alignItems:'flex-start' }}>
              ...
              <div>
                <p style={{ fontFamily:"'DM Mono',monospace", fontSize:'8px', letterSpacing:'0.18em', textTransform:'uppercase', color:'rgb(var(--ink)/0.3)', marginBottom:'3px' }}>{infoLabels[i]}</p>
                <p style={{ fontFamily:"'Jost',sans-serif", fontSize:'13px', fontWeight:400, color:'rgb(var(--ink)/0.65)' }}>
                  <EditableText value={value} docRef={refs.contactVar(code)} isAdmin={isAdmin} as="span" />
                </p>
              </div>
            </div>
          ))}
          ...
          <div>
            <p style={{ fontFamily:"'DM Mono',monospace", fontSize:'8px', letterSpacing:'0.18em', textTransform:'uppercase', color:'rgb(var(--ink)/0.3)', marginBottom:'10px' }}>{c.name ? 'Langues' : 'Languages'}</p>
            <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
              {LANGS_BASE.map((_, i) => (
                <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline' }}>
                  <span style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'14px', fontStyle:'italic', color:'rgb(var(--ink)/0.65)' }}>{langLabels[i]}</span>
                  <span style={{ fontFamily:"'DM Mono',monospace", fontSize:'8px', letterSpacing:'0.1em', color:'rgb(var(--ink)/0.3)' }}>{langLevels[i]}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <motion.div {...up(0.3)}>
          <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'32px' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'20px', flexWrap:'wrap' }}>
              <button type="submit" disabled={status==='sending'} style={{ background:'rgb(var(--ink))', color:'rgb(var(--cream))', padding:'12px 28px', border:'none', fontFamily:"'DM Mono',monospace", fontSize:'9px', letterSpacing:'0.22em', textTransform:'uppercase', opacity:status==='sending'?0.6:1 }}>
                {status==='sending' ? c.sending : c.send}
              </button>
              {status==='success' && <motion.span initial={{opacity:0}} animate={{opacity:1}} style={{ fontFamily:"'DM Mono',monospace", fontSize:'9px', letterSpacing:'0.15em', color:'rgb(var(--rouge))' }}>✓ {c.success}</motion.span>}
              {status==='error'   && <motion.span initial={{opacity:0}} animate={{opacity:1}} style={{ fontFamily:"'DM Mono',monospace", fontSize:'9px', letterSpacing:'0.15em', color:'#dc2626' }}>✕ {c.error}</motion.span>}
            </div>
          </form>
        </motion.div>
      </section>
